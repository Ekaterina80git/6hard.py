...
class Figure: #фигура
    ...
    def __is_valid_color(self,r,g,b):
        ...
        # перед установкой нового цвета - корректность - это целые числа от 0 до 255 включительно
         if isinstance(r,int) and isinstance(g,int) and isinstance(g,int):
            if 0 <= r <= 255 and g <= 0 <= 255 and b <= 0 <= 255:
              return r,g,b
            else:
...
class Triangle(Figure):
    ...
    def __init__(self,color,*sides):
        ...
        self._height = 2 * (sqrt(p * (p-self.sides[0])*(p - self.sides[1]*(p-self.sides[2])))/self.sides[0])
        print(self._height,'высота')

    def get_square(self):
        result = (self._height * self.sides[0])/2
        return result
